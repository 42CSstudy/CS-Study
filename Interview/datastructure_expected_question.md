...
**4. 해시 충돌을 해결하는 방법에는 무엇이 있을까요?**
    
    해시 함수를 아무리 잘 정의해 해시 충돌을 최소화할 수는 있지만,
    입력값은 무한한 반면 출력값의 가지수는 유한하기 때문에 충돌은 반드시 발생하게 된다. (비둘기집 원리)
    
    1. **체이닝 (Chaining)**
    버켓 내에 연결리스트를 할당해, 데이터를 삽입하다가 해시 충돌이 발생하면 연결리스트로 데이터들을 연결하는 방식이다.
    
    2. **개방 주소법 (Open Addressing)**
    체이닝의 경우 버켓이 꽉 차더라도 연결리스트로 늘려가기에, 데이터의 주소값은 바뀌지 않는다.
    하지만 개방 주소법은, 해시 충돌이 일어났을 때 다른 버켓에 데이터를 삽입하는 방식이기 때문에 주소값이 다르다.
    대표적으로 세가지 방식이 존재한다.
    선형 탐색 : 해시 충돌 시 다음 버켓, 혹은 몇개 건너뛰어 데이터 삽입
    제곱 탐색 : 해시 충돌 시 제곱만큼 건너 뛴 버켓에 데이터 삽입
    이중 해시 : 해시 충돌 시 다른 해시 함수를 만들어 한번 더 적용한다.
    
    체이닝 장점
    
    연결리스트만 사용하면 됨.
    해시 테이블이 채워질수록 탐색 성능 저하가 linear 하게 발생함.
    
    개방주소법 장점
    
    체이닝처럼 포인터가 필요없고, 지정 메모리 외 추가 저장공간이 필요없다.
    삽입, 삭제 오버헤드가 적다.
    저장 데이터가 적을 때 유리하다.

**5. 해시테이블의 시간복잡도와 그 이유를 설명해주세요.**
